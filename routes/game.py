...
class Game:
    ...
    def update_state(self):
        hit_players_ids = self.local_player.update(self.game_data.grid, self.players.values())
        if len(hit_players_ids) > 0:
            print(hit_players_ids)
        self.fetch_data_from_server(hit_players_ids)
    ...
    def fetch_data_from_server(self, hit_players_ids):
        if self.is_player_alive:
            self.client.send_player_data_obj(
                PlayerDataObject(self.player_id, self.local_player.rect.x, self.local_player.rect.y,
                                 hit_players_ids, self.updated_hit)
            )
    ...
    def update_players_with_server_data(self):
        ...
        print(self.game_data.players[self.local_player.id].is_hit)
